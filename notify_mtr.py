...
def get_push_config(tz_lst):
    global push_config,notify_function
    push_config = {
    ...
    if tz_lst:
        for item in tz_lst:
            for k,v in item.items():
                if k in push_config.keys() and v:
                    push_config[k] = v
    else:
...
def send(tz_lst,title: str, content: str) -> None:
    get_push_config(tz_lst)
    ts = [
