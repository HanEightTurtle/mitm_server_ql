...
class leafxcy_JavaScript_ksjsb(object):
    def __init__(self,child_conn,temp_folder='temp'):
        # app
        self.app = '快手极速版leafxcy'
        # result of capture
        self.res = {
            'ksjsbCookie':''
        }
    ...
    def request(self,flow: mitmproxy.http.HTTPFlow):
        ...
        if self.count == 0:
            if any( [re.search(pattern,str(flow.request.pretty_url)) for pattern in ['api.kuaisho.*?\.com','open.kuaisho.*?\.com'] ] ):
                ...
                if params:
                    if set(['kuaishou.api_st']) < set(params.keys()):
                        self.res['ksjsbCookie'] = f"kuaishou.api_st={params.get('kuaishou.api_st')};"
                else:
                    body = data_str_to_dict(flow.request.get_text())
                    if set(['kuaishou.api_st']) < set(body.keys()):
                        self.res['ksjsbCookie'] = f"kuaishou.api_st={body.get('kuaishou.api_st')};"
            # check res and save
